...
evalPredicate :: Set a -> Predicate a
evalPredicate EmptySet = const False
evalPredicate (PropSet op) = case op of
  Insert predicate s -> \x -> x `member` s || predicate x
  Delete predicate s -> \x -> x `member` s && (not $ predicate x)
...
union s1 s2 = PropSet $ Insert (`member` s1) s2

-- | Difference of two sets.
difference :: Set a -> Set a -> Set a
difference s1 s2 = PropSet $ Insert (`notMember` s2) s1

-- | Intersection of two sets.
intersection :: Set a -> Set a -> Set a
intersection s1 s2 = PropSet $ Insert (\x -> x `member` s1 && x `member` s2) empty
